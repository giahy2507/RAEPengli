...
from nn.uRAE import UnfoldingRecursiveAutoencoder
...
def compute_cost_and_grad(theta, instances, total_internal_node_num,
  ...
  if rank == 0:
    ...
    rec_error, gradient_vec = process_local_batch(uRAE, word_vectors, instances)
    
    # compute total reconstruction error
    total_rec_error = comm.reduce(rec_error, op=MPI.SUM, root=0)
    
    # compute total cost
    reg = uRAE.get_weights_square()
    ...
    reg_grad = uRAE.get_zero_gradients()
    reg_grad.gradWi1 += uRAE.Wi1
    reg_grad.gradWi2 += uRAE.Wi2
    reg_grad.gradWo1 += uRAE.Wo1
    reg_grad.gradWo2 += uRAE.Wo2
    reg_grad *= lambda_reg
    ...
    while True:
      ...
      # init recursive autoencoder
      #urae = RecursiveAutoencoder.build(theta, embsize)
      uRAE = UnfoldingRecursiveAutoencoder.build(theta, embsize)
...
def process_local_batch(urae, word_vectors, instances):
  gradients = urae.get_zero_gradients()
  total_rec_error = 0
  for instance in instances:
    words_embedded = word_vectors[instance.words]
    encode_tree, decode_tree, rec_error = urae.forwardUnfolding(words_embedded)
    urae.backwardUnfolding(encode_tree, decode_tree, gradients, instance.freq)
...
if __name__ == '__main__':
  ...
  parser.add_argument('-instances', required=True,help='instances for training')
  parser.add_argument('-model', required=True,help='model name')
  parser.add_argument('-word_vector', required=True,help='word vector file',)
  parser.add_argument('-lambda_reg', type=float, default=0.15,help='weight of the regularizer')
  parser.add_argument('--save-theta0', action='store_true',help='save theta0 or not, for dubegging purpose')
  parser.add_argument('--checking-grad', action='store_true', help='checking gradients or not, for dubegging purpose')
  parser.add_argument('-m', '--maxiter', type=int, default=100,help='max iteration number',)
  parser.add_argument('-e', '--every', type=int, default=0,help='dump parameters every --every iterations',)
  parser.add_argument('--seed', default=None,help='random number seed for initialize random',)
  parser.add_argument('-v', '--verbose', type=int, default=0,help='verbose level')
  options = parser.parse_args()
  '''
  instances_file = '../sample-training-file.txt'
    #options.instances
  model = '../sample-training-file.mpi-1.model.gz'
    #options.model
  word_vector_file = '../sample-word-vectors-trained-by-word2vec.txt'
    #options.word_vector
  lambda_reg = 0.15
    #options.lambda_reg
